...
@section('title', 'Reeditar Formulário Prodist')
@section('content')
  @php
  ...
  <div class="container-step-content">
    <form class="container" method="POST" id="regForm" action="{{ route('prodist.edit', $prodist) }}" enctype="multipart/form-data">
      @csrf
      @method('post')
  ...
  <script>
    $(function() {
     // $('.fieldset_disabled').prop('disabled', true);
      $('.fieldset_prodist').prop('disabled', {{ $eh_apoiador }});
      ...
      document.getElementById('titular').onclick = function() {
        if (this.checked) {
          $('.fieldset_prodist').prop('disabled', false);
        }
      };
      document.getElementById('ntitular').onclick = function() {
        if (this.checked) {
          $('.fieldset_prodist').prop('disabled', true);
        }
      };
